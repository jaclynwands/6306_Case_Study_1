...
## **We should also discuss why we are choosing to not combine data at this exact moment (we will do it later and KNN to predict the missing values) ** ##
```{r}
...
## **This graph should be ordered from lowest to highest and a color gradient that follows the same code would be a great visual to help our users ** ##
```{r}
...
stateABV_associated
```
### The States with the highest median ABV is DC and KY at 6.45%.
...
# ** We use the same analysis above but by Beer Type, this will allow us to advise Budweiser on the Type, IBU, ABV of a new beer to release to compete with the growing market. ** #
