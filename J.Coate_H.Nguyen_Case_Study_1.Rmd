...
## Number of breweries are present in each state? 
...
#### Number of breweries are present in each state? (Wev give exact number and after that we show by using bat plot for visualization. What do you think?)
...
#### Huy: I don't want to create only 1 df for ABV and IBU because if we only consider the median of ABV, we only need to delete 62 NA and if we want to calculate IBU, we only need to delete 1005NA. That's why in my following codes for Question 4, I created 2 separated df, one for ABV and one for IBU.
...
#Huy's comment:
I don't want to create only 1 df for ABV and IBU because if we only consider the median of ABV, we only need to delete 62 NA and if we want to calculate IBU, we only need to delete 1005NA. That's why in my following codes for Question 4, I created 2 separated df, one for ABV and one for IBU.
...
## Huy's comment: 
It's the maximum ABV, not the median. then we use this code
brew.beer
brew.beer[which.max(brew.beer$ABV),]
...
I want to add more to  my last code here. The last code for summary statistics anf distributions on median and per state of ABV and here I want to give a genenral sumary

summary(brew.beer.cleanABV$ABV)
...
brew.beer %>% ggplot(mapping = aes(x=IBU, y=ABV)) +
  ...
  ggtitle("Relationship between the bitterness of the beer and its alcoholic content") + ylab("ABV") + xlab("IBU")

```
```

## Jaclyn: The scatter plot

## Huy: Complete this scatter plot
