...
brew.beerIBU <- aggregate(brew.beer.cleanABV$IBU, by=list(brew.beer.cleanIBU$State), FUN = median)
...
```{r Turn IBU into percent which is standard format for IBU}

brew.beerIBU <- mutate(brew.beerIBU, IBUper = MedIBU * 100)
...
# Alcohol by Volume (IBU) Analysis
...
```{r IBU into percent which is normal format for IBU}

maxIBU <- mutate(maxIBU, IBUper = IBU * 100)
...
colorMaxIBU <- hcmap("countries/us/us-all", data = maxIBU, value = "IBUper",
...
hc_colorAxis(colorMaxIBU, min = 3,
...
- 75% of the ABV falls between 30 - 43
